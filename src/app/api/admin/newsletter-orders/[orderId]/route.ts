...
import { ensureCommunityMembershipForNewsletterOrder } from "@/lib/community-access";
...
export async function PATCH(
  ...
  try {
    ...
    const order = await prisma.newsletterOrder.update({
      where: { id: orderId },
      data: {
    ...
    return NextResponse.json({ error: "Unable to review newsletter order." }, { status: 500 });
  }
}
