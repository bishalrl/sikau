...
export function CombinedPaymentQueue({
  ...
  newsletterOrders?: NewsletterOrderItem[];
}) {
  const [items, setItems] = useState(ebookOrders);
  const [newsletterItems, setNewsletterItems] = useState(newsletterOrders);
  ...
  async function reviewNewsletter(orderId: string, status: "APPROVED" | "REJECTED") {
    setMessage("");
    ...
    setNewsletterItems((current) => current.filter((item) => item.id !== orderId));
      ...
      <section className="space-y-4">
        ...
        {newsletterItems.map((order) => (
          <div key={order.id} className="rounded-3xl border border-outline-variant/30 bg-white p-5">
            ...
            <div className="mt-4 flex flex-wrap gap-3">
              <Button type="button" onClick={() => void reviewNewsletter(order.id, "APPROVED")}>
                Approve
              </Button>
              <Button
                type="button"
                variant="outline"
                onClick={() => void reviewNewsletter(order.id, "REJECTED")}
              >
                Reject
              </Button>
            </div>
        ...
        {newsletterItems.length === 0 && (
          <p className="text-sm text-on-surface-variant">No pending newsletter payments.</p>
        )}
